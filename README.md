...
**Deployed and running on the Raspberry Pi 2B** (`rpi2b.local`) as a systemd service (`fpv-tuner.service`), verified reachable at `http://rpi2b.local:8000`. Full test suite (148 tests) passes on-device (real ARMv7/Python 3.13 hardware). The full appliance workflow is implemented end-to-end: FC auto-detection, connect, Blackbox retrieval directly off the FC's onboard flash over MSP, analysis, a rule-based tuning-recommendation engine, and a safety-gated apply-to-hardware flow with backup/verify/rollback — all wired to a state-machine-driven, card-paginated touchscreen UI built for the real 320×240 panel. Verified live: FC connect/version/target detection, and kiosk-mode rendering (via real framebuffer screenshots). **Not yet verified against real hardware**: the actual MSP Blackbox-download and tuning-apply write paths (built and unit-tested with mocks only — see [Safety](#safety)), and analysis output hasn't been cross-validated against a real *sane* flight log yet. See [Roadmap](#roadmap).
